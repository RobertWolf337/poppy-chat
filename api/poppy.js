...
// - Uses kit.json + book.json
// - Persona support: max / harvey / aurora (default Poppy)
// - UK English, kid-safe tone
// - Returns { reply }
...
async function loadKit(req) {
  ...
  try { const resp = await fetch(url, { cache: "no-store" }); if (resp.ok) KIT_CACHE = await resp.json(); } catch {}
  return KIT_CACHE;
}
...
async function loadBook(req) {
  ...
  try { const resp = await fetch(url, { cache: "no-store" }); if (resp.ok) BOOK_CACHE = await resp.json(); } catch {}
  return BOOK_CACHE;
}
function findBookSnippets(book, q = "") {
  ...
  for (const sec of book.sections) {
    ...
    let hits = 0; for (const t of tokens) if (hay.includes(t)) hits++;
    if (hits >= Math.min(2, tokens.length)) { out.push((sec.text || "").slice(0, 400)); if (out.length >= 3) break; }
  }
  return out;
}
...
`.trim();
    ...
    // booklet grounding (light)
    const book = await loadBook(req);
    const bookBits = findBookSnippets(book, q);
    const BOOK_CTX = bookBits.length
      ? `\n\nHelpful notes from our booklet:\n- ${bookBits.join("\n- ")}\n\nUse these notes only if helpful; keep the answer short.`
      : "";

    // messages
    const messages = [{ role: "system", content: SYSTEM_PROMPT + BOOK_CTX }];
